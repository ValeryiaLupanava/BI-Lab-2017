...
BEGIN
   ...
   SELECT bl_dm.fct_retail_sales_dd_seq.NEXTVAL AS sale_id,
             ...
             a.insert_dt,
             a.update_dt 
      FROM   cls_fct_retail_sales_dd a LEFT JOIN bl_dm.dim_customers_scd b ON a.customer_surr_id = b.customer_id
                                       LEFT JOIN bl_dm.dim_employees_scd c ON a.employee_surr_id = c.employee_id
                                       LEFT JOIN bl_dm.dim_stores_scd d ON a.store_surr_id = d.store_id
                                       LEFT JOIN bl_dm.dim_payment_methods_scd d ON a.payment_method_surr_id = d.payment_method_id
                                       LEFT JOIN bl_dm.dim_products_scd e ON a.product_surr_id = e.product_id;
    COMMIT;
EXCEPTION
WHEN OTHERS THEN
    RAISE;
